...
//     void Utility::print_result1(const std::vector<OCRPredictResult> &ocr_result) {
...
//     }
    ...
    void Utility::print_result(const std::vector<OCRPredictResult>& ocr_result) {
        ...
        for (std::size_t i = 0; i < ocr_result.size(); i++) {
            ...
            // rec
            if (ocr_result[i].score != -1.0) {
                output_file << " rec text: " << ocr_result[i].text << " " << std::endl
                            << " rec score: " << ocr_result[i].score << " ";
            }

            // cls
            if (ocr_result[i].cls_label != -1) {
                output_file << " cls label: " << ocr_result[i].cls_label
                            << " cls score: " << ocr_result[i].cls_score;
            }
